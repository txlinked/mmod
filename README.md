...
Numeric repeater IDs use RadioID's separate official repeater directory. Repeater entries show their registered callsign and location, plus the registered callsign’s first name when present in the subscriber directory. This identifies the registration, not necessarily the person transmitting. Subscriber identities take precedence. Weekly refreshes also repair saved last-caller and activity records without requiring a new call. Unknown IDs remain numeric. Updates request a directory refresh automatically.
